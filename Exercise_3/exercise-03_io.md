Write a programm (nodeJs CLI-Script), that uses a single **synchronous** filesystem operation to read a file and print the number of newlines (\n) it contains to the console (stdout), similar to running `cat file | wc -l`.
...
>You should start with an research about node modules (node core modules and **custom** modules) and how to use them in an nodejs programm.
